...
class Lemmatizer(Preprocessor):
    ...
    def _get_wordnet_pos(self, tweet):
        ...
        changed_pos = dict([(name, self.tag_dict.get(pos)) for name, pos in pos.items()])
    ...
    def _get_values(self, inputs):
        ...
        for tweet in inputs[0]:
            ...
            row = [self.lemmatizer.lemmatize(word, pos) if pos else self.lemmatizer.lemmatize(word) for word,pos in pos_tweet.items()]
